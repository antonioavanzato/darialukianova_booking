...
yc serverless function version create \
  ...
  --source-path fn.zip \
  --environment "YDB_ENDPOINT=$YDB_ENDPOINT,YDB_DATABASE=$YDB_DATABASE,ADMIN_PASSWORD=$ADMIN_PASSWORD,JWT_SECRET=$JWT_SECRET${ADMIN_LOGIN:+,ADMIN_LOGIN=$ADMIN_LOGIN}${TELEGRAM_TOKEN:+,TELEGRAM_TOKEN=$TELEGRAM_TOKEN}${TELEGRAM_CHAT_ID:+,TELEGRAM_CHAT_ID=$TELEGRAM_CHAT_ID}${VAPID_PUBLIC_KEY:+,VAPID_PUBLIC_KEY=$VAPID_PUBLIC_KEY}${VAPID_PRIVATE_KEY:+,VAPID_PRIVATE_KEY=$VAPID_PRIVATE_KEY}${VAPID_SUBJECT:+,VAPID_SUBJECT=$VAPID_SUBJECT}"
rm -f fn.zip
